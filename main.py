...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): keyboard = [["📦 মেনু"]] reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True) await update.message.reply_text( "✨ স্বাগতম Zentix Ai Bot ৳এ!", parse_mode="Markdown" ) await update.message.reply_text( "🤖 আমি আপনার ব্যক্তিগত AI সহকারীী\n\n" "📦 মেনু দেখতে নিচের '📦 মেনু' বাটনে ক্লিক করুন —\n" "নতুন প্রয়ুক্তির সাথে যুক্ত হোন এক ক্লিকে।", reply_markup=reply_markup )
...
if page == 1:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("⬅️ পেছনের পেজ", callback_data="page_1"), InlineKeyboardButton("➡️ পেজ ৮", callback_data="page_3")]
    ]
else:
    keyboard = [
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE): data = update.callback_query.data if data.startswith("page_"): page_num = int(data.split("_")[1]) await send_page(update, context, page_num) else: await update.callback_query.answer("🚧 ফিচারটি এখনো ত্যারি হচ্ছে!")
...
✅ যেকোনো বার্তায় যদি 'মেনু' থাকে

app.add_handler(MessageHandler(filters.TEXT & filters.Regex(r"(\ud83d\udce6\s*)?মেনু"), menu))
